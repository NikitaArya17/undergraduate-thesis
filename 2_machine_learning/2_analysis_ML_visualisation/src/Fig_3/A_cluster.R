...
#install.packages(c("FSelectorRcpp", "randomForest", "glmnet", "pls", "caret", "apcluster"))

#FSelector package replaced with FSelectorRcpp as this package offers 
#speed and performance enhancements,
#and, unlike its predecessor, does not require a Java installation on the computer
#on which it is run. This makes the code portable and reproducible.
...
p2 = dim(df)[2]
mut = df[,(cul_f_last+2):p2]

dep= read.csv("2_analysis_ML_visualisation/output_data/dependence_repro.csv", header=TRUE,fill=TRUE,stringsAsFactors=FALSE)
...
write.csv(cluster,"2_analysis_ML_visualisation/output_data/cluster.csv")
...
write.csv(data.frame(gr,top_clus),"2_analysis_ML_visualisation/output_data/top_clus_18.csv",row.names=FALSE)
...
write.csv(cluster,"2_analysis_ML_visualisation/output_data/cluster_18.csv",row.names = FALSE)
...
write.csv(yanse,"2_analysis_ML_visualisation/output_data/colors.csv",row.names=FALSE)
...
# 
#plot the first row
png(file = paste("2_analysis_ML_visualisation/output_data/Origin", q, ".png",sep=""),width = 3000, height = 3000, res= 800,
