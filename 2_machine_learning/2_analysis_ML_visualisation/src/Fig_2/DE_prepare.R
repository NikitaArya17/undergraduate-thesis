...
d1= read.csv(file.path("2_analysis_ML_visualisation/ref/MGgenelist.csv"), header=TRUE,stringsAsFactors=FALSE,check.names=FALSE)
...
output[non_zero,2] = 0 #removed (-) sign; can't include and exclude in the same subsetting step
output[is.na(output[non_zero,2]) & !is.na(output[non_zero,2])] = out[,2]
#line rewritten to solve error: NAs are not allowed in subscripted assignments
