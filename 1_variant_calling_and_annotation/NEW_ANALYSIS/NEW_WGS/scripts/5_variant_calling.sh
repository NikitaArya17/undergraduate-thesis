...
# First, create a reference dictionary that the tool can use
gatk CreateSequenceDictionary -R ${REF_GENOME}

#Sort the input file
samtools sort -@ 4 -o ${ALIGN_DIR}/aln_dedup_sorted.bam ${ALIGN_DIR}/aln_dedup.bam

#Index the input file first
samtools index ${ALIGN_DIR}/aln_dedup_sorted.bam

gatk HaplotypeCaller -ploidy 4 -R ${REF_GENOME} \
-I ${ALIGN_DIR}/aln_dedup_sorted.bam \
-O ${VARIANT_DIR}/raw_variants.vcf

gatk SelectVariants -R ${REF_GENOME} \
-V ${VARIANT_DIR}/raw_variants.vcf -select-type SNP \
-O ${VARIANT_DIR}/raw_snps.vcf

gatk SelectVariants -R ${REF_GENOME} \
-V ${VARIANT_DIR}/raw_variants.vcf -select-type INDEL \
-O ${VARIANT_DIR}/raw_indels.vcf
